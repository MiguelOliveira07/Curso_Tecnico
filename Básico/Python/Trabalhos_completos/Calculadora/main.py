...
def ip_host(meu_ip):
    return ip.IPv4Address(meu_ip)


def ip_rede(meu_ip):
    rede = ip.IPv4Network(meu_ip, strict=False)
    info = [
        f"Endereço de rede: {rede}",
        f"Broadcast: {rede.broadcast_address}",
        f"IP da rede: {rede.network_address}",
        f"Máscara: {rede.with_netmask}",
    ]
    return rede.network_address, info
...
def main(page: ft.Page):
    ...
    def add_clicked(e):
        ...
        if not meu_host or not meu_rede:
            page.snack_bar = ft.SnackBar(ft.Text("Informe o IP do host e da rede."), bgcolor=ft.Colors.RED_400)
            page.snack_bar.open = True
        else:
            info = verificar_host(meu_host, meu_rede)
            if len(info) == 1 and info[0].startswith("Erro:"):
                page.snack_bar = ft.SnackBar(ft.Text(info[0]), bgcolor=ft.Colors.RED_400)
                page.snack_bar.open = True
            else:
                for item in info:
    ...
    new_rede = ft.TextField(
        hint_text="Entre com um IP de REDE válido:",
        width=350,
        border_color=ft.Colors.BLUE_400,
    )

    add_button = ft.FloatingActionButton(
        icon=ft.Icons.ADD, bgcolor=ft.Colors.BLUE_400, on_click=add_clicked
    )

    page.add(
        ft.Column(
            [
                ft.Text("Verificador de Host e Rede", size=20, weight=ft.FontWeight.BOLD),
                new_host,
                new_rede,
                add_button,
                ft.Divider(),
